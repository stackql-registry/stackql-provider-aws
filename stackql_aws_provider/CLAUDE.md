...
- **Existing rows win**: stage 2 uses the row's `stackql_resource_name` / `stackql_method_name` / `stackql_verb` / `stackql_object_key` over the breadcrumb-derived values, and logs each divergence (`pin` lines). Pins are resolved onto the breadcrumbs BEFORE any resource-scoped pass (NOCASE demotion, identifier promotion, signature dedupe) so those passes group by the CSV resource, not the derived one - otherwise an op the CSV moved elsewhere still feeds its columns and required params into the derived resource (glue RunStatement vs GetStatement).
- **Output must equal the CSV**: after each service is built, stage 2 asserts every pinned row came out as exactly that resource.method with that effective verb (a method in no sqlVerbs array is `exec`). Any divergence - a pinned select demoted by the signature dedupe or the zero-column pass, a pinned method that vanished - is a hard build failure naming the rows. Resolve it in the CSV (or `param_promotions.json`), never by tolerating the drift.
- **Lint on load**: duplicate `filename::operationId` keys, unknown verbs, rows that are neither fully populated nor `skip`, GET_/POST_ twins that disagree, and two ops claiming one `(resource, method)` all fail the build before anything is written.
- **`skip` is a verb**: `stackql_verb=skip` catalogues an operation without routing it (no method is emitted). It is the only way to exclude an op; blanking a row is a lint failure.
- **Complete catalog**: the CSV holds a row for every operation botocore declares, not just the ops stage 1 emits. After the build, a catalog pass walks `botocore/data` directly and appends `skip` rows (reason in `op_description`) for anything without one - e.g. the cbor-only `partnercentral_revenue_measurement` service. Rows whose op botocore no longer declares are reported as stale and kept.
- **Pin inheritance**: a new op whose breadcrumb resource matches the breadcrumb resource of pinned ops in the same file, where those ops all pin to one CSV resource, is proposed under that CSV resource (only non-EXEC siblings anchor this - EXEC ops are usually placed by co-location heuristics, not entity identity). A conscious rename or collapse in the CSV therefore propagates to later botocore additions instead of resurrecting the old name (`inherit` lines in the log, `inherited` in the build report).
- **`--strict`** (CI): fail if any operation has no CSV row instead of appending. Local regen runs without it, appends, and the diff is reviewed and committed.
- **Append-only**: rows are never rewritten. New `filename::operationId` keys get derived mappings and are appended; rows for removed ops persist harmlessly (reported as stale). Renames, collapses and verb changes are edits to existing rows - that IS the workflow.
...
**Forced param promotions** — `provider-dev/config/param_promotions.json` (committed, same conscious-edit convention as the CSV): `{"<filename>::<OperationName>": ["ParamName", ...]}` forces the named typed params to `required: true` at stage 2 (GET_/POST_ operationId prefix ignored, so one entry covers both query/ec2 twins). Used when a CSV co-location merge puts two listings with identical required-param signatures in one SELECT bucket and identifier promotion can't split them — e.g. `ec2.instance_types` merges `DescribeInstanceTypes` (`[]`) and `DescribeInstanceTypeOfferings` (`[]`); requiring `LocationType` on the offerings op makes the signatures diverge so both stay selectable. Runs before `demoteNocaseCollisions` so the NOCASE passes and build guards see the promoted view; applications are logged in `build-report.json` under `forcedPromotions`.

**Benchmark regression guard** — `provider-dev/config/benchmarks.json` (committed) pins the surface counts: services / resources / methods / selectableResources must never FALL between generations (they only creep up as botocore grows), and the non-selectable ratio must not spike beyond `ratioTolerancePct` (a spike means a generator change started stranding readers). Stage 2 hard-fails on any regression; conscious re-baselining is `generate-provider.mjs --update-benchmarks`. `execOnlyResources` is tracked alongside. The 2026-09-18 re-baseline (6199 -> 6967 resources, 18152 -> 19092 methods) reflects exec-only resources being kept rather than pruned.
...
- **Empty output structures emit no response content** - a structure with no members (`CancelPolicyGenerationResponse`) is treated exactly like a no-output op: `200: {description: Success}` with no `content`. A bare `type: object` response makes the EXEC analyzer fail with `schema unsuitable for select query`, and `properties: {}` does not help (verified on v0.11.669). Implementation: `_effective_output()` in the Python generator.
...
`VERB_PREFIXES` in `botocore_to_openapi.py`. EXEC is reserved for **lifecycle / state-machine transitions** (Start, Stop, Reboot, Run, Enable, Disable, Activate, Deactivate, Accept, Reject, Reset, Restore, Import, Export, Copy, Move, Migrate, ...). UPDATE covers **field / relation changes** (Update, Modify, Patch, Set, Tag, Untag, Add, Remove, Attach, Detach, Associate, Disassociate, Assign, Unassign, Authorize, Revoke, Allocate, Release, ...). SELECT verbs: Describe, List, BatchGet, Get, Search, Lookup. **Every other leading CamelCase token is treated as a verb** (Check, Detect, Request, Execute, ...) and mapped to EXEC on the remaining noun - a resource is a noun, never an action name (`CheckNoPublicAccess` -> `policies.check_no_public_access` by CSV override, `DetectEntities` -> `entities.detect_entities`). `Batch*` ops are EXEC on the noun's entity (no SQL batch mutations); `BatchGet*` stays SELECT. Connectors are filter qualifiers, not name parts: known-verb ops keep the entity BEFORE By/For (`ListFunctionsByCodeSigningConfig` -> `functions`, overloaded by required-param signature), generic-verb and Batch ops drop leading connectors and keep the part before By/With and after For/To/Into/From (`ExchangeCodeForToken` -> `tokens`). Methods folded into one resource this way MUST differ in required params (path or query); the remap script and the build guard both refuse a merge that would clash.

### 7. Resource grouping — pluralise, co-locate CRUD

Resource name = pluralised noun after verb-prefix strip. `DescribeVolumes` → `volumes`, `CreateVolume` → `volumes`, `AttachVolume` → `volumes`. Full CRUD-and-lifecycle co-located. Every resource name is a pluralised noun, EXEC ops included (`StartPolicyGeneration` -> `policy_generations`). The pluraliser is version-suffix aware (`FindingV2` -> `findings_v2`, never `finding_v2s`), leaves uncountables alone (`metric_data`, `account_information`), and knows the `s`-ending singulars that take `es` (`alias` -> `aliases`, `status` -> `statuses`, `bus` -> `buses`, `address` -> `addresses`); other `s`-ending tokens are assumed plural. Implementation: `_pluralise_word` / `_pluralise` in the Python generator.
...
2. **Full-suffix merge** — a non-selectable resource whose name ends with a selectable sibling's name folds into it, longest match first (`application_fleets` -> `fleets`, `scaling_policies` -> `policies`). Rule 2b: a resource whose pluralised whole name is a selectable sibling folds into it (`policy_generation` -> `policy_generations`).
...
Within each `sqlVerbs[verbKey]` array the order is a **subset-aware partial order, primary methods first**. The router picks the first method whose required params are satisfiable from the SQL clause, so a method must precede every method whose signature is a STRICT SUBSET of its own: `get_function` (`[function_name, region]`) sits ahead of `list_functions` (`[region]`). Methods with unrelated signatures are free to order, and because DESCRIBE resolves a resource's columns from the FIRST select method, the resource's primary methods (noun matches the resource) win those free choices - a folded-in qualifier lister (`list_hosted_zones_by_vpc` `[vpc_id, vpc_region]`) must not displace `get_hosted_zone` as the describe anchor just because it has more required params. (A plain `requiredParams.length DESC` sort did exactly that after the `_by_` folds and failed the route53 describe test.)

### 9. `sqlVerbs` uniqueness — no duplicate signatures

Within a `(resource, sqlVerb)` bucket, no two methods share the same required-params signature. Stage 2 has a build-time `verifySignatureUniqueness` guard that hard-fails the build if any collision survives the dedupe. Note: the candidates phase records `requiredParams` from the **final** registered op (the one whose `$ref` `methodEntry.operation.$ref` points at), not the first iteration — so dedupe matches what `verifySignatureUniqueness` sees.

**SELECT clashes: identifier promotion BEFORE demotion.** When a point-read (`get_user`, `describe_stream`) collides with the listing (`list_users`) because botocore marks its identifier optional (IAM GetUser defaults UserName to the caller), the contract is that the point-read REQUIRES its identifier: `promoteIdentifierParam` sets the op's optional identifier param (`singular(resource)` + `name|id|arn|identifier`, separator/case-insensitive, in typed params or the runtime-bound body schema) to `required`, the signatures diverge, and BOTH stay selectable — `WHERE UserName = 'x'` routes to `get_user`, bare SELECT to `list_users`. Promotion refuses params that would NOCASE-collide with the resource's response columns (cloudformation `types.describe_type` param `Type` vs column `type`). Unresolvable clashes still demote: winner is `pickSelectAnchor` — `list_*` > primary > any, with ALL pinned tiers outranking unpinned so a new botocore op never evicts a pinned incumbent (and a `batch_get_*` never steals the bare-SELECT slot on lexical accident). Non-select buckets keep the old precedence (pin > primary > insertion). Every run writes `provider-dev/config/build-report.json` (promotions/demotions/zero-column) for CSV reconciliation.

### 10. Exec-only resources are kept; zero-column selects demote

Exec-only resources (all sqlVerb arrays empty) are KEPT. Verified on stackql v0.11.669: `SHOW RESOURCES` lists them, `SHOW EXTENDED METHODS` lists their methods with SQLVerb `EXEC`, `DESCRIBE` answers "SELECT not supported for this resource", and `EXEC` routes. The old empty-sqlVerbs prune silently dropped ~1000 CSV-catalogued operations (`StartPolicyGeneration`, `ImportCertificate`, `RequestCertificate`, all of `rds_data`) and broke the CSV -> output guarantee. Only a service with no resources at all (every op `skip`) is left out of `providerServices`. test-meta-routes runs DESCRIBE only on selectable resources, so exec-only resources pass it.
...
- Compares names **as the runtime renders them** (`toSnakeAlias`, the port of any-sdk `casing.ToSnake`): under `snake_case_aliases` SHOW METHODS reports RequiredParams in snake form, so a required body field `branchName` and a path param `branch_name` are the SAME parameter to the router. A wire-name comparison let 31 pinned methods through that `test-meta-routes` then rejected at runtime (amplify `create_branch` vs `create_deployment`, connect `search_*` vs `list_*`); they are now pinned `exec` in the CSV.
