...
<tr><td><b>Id</b></td><td><CopyableCode code="aws.waf_regional.rule_groups" /></td></tr>
</tbody></table>
...
<tr>
    <td><CopyableCode code="rule_group_id" /></td>
    <td><code>string</code></td>
...
name,
rule_group_id
FROM aws.waf_regional.rule_groups
...
```sql
INSERT INTO aws.waf_regional.rule_groups (
Name,
MetricName,
ChangeToken,
Tags,
...
```sql
UPDATE aws.waf_regional.rule_groups
SET 
...
```sql
DELETE FROM aws.waf_regional.rule_groups
WHERE region = '{{ region }}' --required
;
```
</TabItem>
