...
```sql
INSERT INTO aws.lexv2_models.bot_aliases (
botAliasName,
description,
botVersion,
...
- name: bot_aliases
  props:
    - name: bot_id
      value: "{{ bot_id }}"
      description: Required parameter for the bot_aliases resource.
    - name: region
      value: "{{ region }}"
      description: Required parameter for the bot_aliases resource.
    - name: botAliasName
...
```sql
UPDATE aws.lexv2_models.bot_aliases
SET 
...
```sql
DELETE FROM aws.lexv2_models.bot_aliases
WHERE bot_alias_id = '{{ bot_alias_id }}' --required
AND bot_id = '{{ bot_id }}' --required
AND region = '{{ region }}' --required
