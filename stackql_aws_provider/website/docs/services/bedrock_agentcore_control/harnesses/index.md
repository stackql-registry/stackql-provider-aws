...
```sql
INSERT INTO aws.bedrock_agentcore_control.harnesses (
harnessName,
clientToken,
executionRoleArn,
environment,
...
- name: harnesses
  props:
    - name: region
      value: "{{ region }}"
      description: Required parameter for the harnesses resource.
    - name: harnessName
      value: "{{ harnessName }}"
    - name: clientToken
      value: "{{ clientToken }}"
...
```sql
UPDATE aws.bedrock_agentcore_control.harnesses
SET 
...
```sql
DELETE FROM aws.bedrock_agentcore_control.harnesses
WHERE harness_id = '{{ harness_id }}' --required
AND region = '{{ region }}' --required
