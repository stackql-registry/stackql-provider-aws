...
<tr><td><b>Id</b></td><td><CopyableCode code="aws.support_app.slack_workspaces" /></td></tr>
</tbody></table>
...
```sql
INSERT INTO aws.support_app.slack_workspaces (
teamId,
region
)
SELECT 
...
- name: slack_workspaces
  props:
    - name: region
      value: "{{ region }}"
      description: Required parameter for the slack_workspaces resource.
    - name: teamId
      value: "{{ teamId }}"
`}</CodeBlock>
