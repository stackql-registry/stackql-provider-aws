...
<tr><td><b>Id</b></td><td><CopyableCode code="aws.qconnect.content" /></td></tr>
</tbody></table>
...
```sql
INSERT INTO aws.qconnect.content (
name,
title,
overrideLinkOutUri,
metadata,
...
```sql
DELETE FROM aws.qconnect.content
WHERE knowledge_base_id = '{{ knowledge_base_id }}' --required
AND content_id = '{{ content_id }}' --required
AND region = '{{ region }}' --required
;
...
```sql
EXEC aws.qconnect.content.start_content_upload 
@knowledge_base_id='{{ knowledge_base_id }}' --required, 
