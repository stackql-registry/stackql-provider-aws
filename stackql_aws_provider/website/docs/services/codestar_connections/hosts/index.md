...
<tr><td><b>Id</b></td><td><CopyableCode code="aws.codestar_connections.hosts" /></td></tr>
</tbody></table>
...
```sql
INSERT INTO aws.codestar_connections.hosts (
Name,
ProviderType,
ProviderEndpoint,
VpcConfiguration,
...
```sql
UPDATE aws.codestar_connections.hosts
SET 
HostArn = '{{ HostArn }}',
...
```sql
DELETE FROM aws.codestar_connections.hosts
WHERE region = '{{ region }}' --required
;
```
</TabItem>
