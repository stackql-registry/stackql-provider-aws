...
```sql
INSERT INTO aws.connect.contact_flow_module_aliases (
Description,
...
- name: contact_flow_module_aliases
  props:
    - name: instance_id
      value: "{{ instance_id }}"
      description: Required parameter for the contact_flow_module_aliases resource.
...
```sql
UPDATE aws.connect.contact_flow_module_aliases
SET 
Name = '{{ Name }}',
...
```sql
DELETE FROM aws.connect.contact_flow_module_aliases
WHERE instance_id = '{{ instance_id }}' --required
AND contact_flow_module_id = '{{ contact_flow_module_id }}' --required
AND alias_id = '{{ alias_id }}' --required
AND region = '{{ region }}' --required
