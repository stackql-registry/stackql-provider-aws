...
<tr><td><b>Id</b></td><td><CopyableCode code="aws.wisdom.content" /></td></tr>
</tbody></table>
...
```sql
INSERT INTO aws.wisdom.content (
clientToken,
metadata,
name,
overrideLinkOutUri,
...
```sql
DELETE FROM aws.wisdom.content
WHERE content_id = '{{ content_id }}' --required
AND knowledge_base_id = '{{ knowledge_base_id }}' --required
AND region = '{{ region }}' --required
;
...
```sql
EXEC aws.wisdom.content.start_content_upload 
