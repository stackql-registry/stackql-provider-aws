...
```sql
INSERT INTO aws.verifiedpermissions.policy_store_aliases (
aliasName,
policyStoreId,
region
)
...
- name: policy_store_aliases
  props:
    - name: region
      value: "{{ region }}"
      description: Required parameter for the policy_store_aliases resource.
    - name: aliasName
      value: "{{ aliasName }}"
      description: |
...
```sql
DELETE FROM aws.verifiedpermissions.policy_store_aliases
WHERE region = '{{ region }}' --required
;
```
</TabItem>
