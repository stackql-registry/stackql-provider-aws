...
```sql
INSERT INTO aws.transfer.accesses (
HomeDirectory,
HomeDirectoryType,
...
- name: accesses
  props:
    - name: region
      value: "{{ region }}"
      description: Required parameter for the accesses resource.
    - name: HomeDirectory
      value: "{{ HomeDirectory }}"
      description: |
...
```sql
DELETE FROM aws.transfer.accesses
WHERE region = '{{ region }}' --required
;
```
</TabItem>
